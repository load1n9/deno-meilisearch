...
import { Config, EnqueuedTaskObject } from "../types.ts";
import {
  httpErrorHandler,
...
import { addProtocolIfNotPresent, addTrailingSlash } from "./utils.ts";

type queryParams<T> = { [key in keyof T]: string };

// deno-lint-ignore ban-types
export function toQueryParams<T extends object>(parameters: T): queryParams<T> {
...
function createHeaders(config: Config): Record<string, any> {
  ...
  const packageAgent = `Meilisearch Deno`;
