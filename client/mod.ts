...
import { HttpRequests } from "./httpRequests.ts";
import { Task, TaskClient } from "./Task.ts";
import { EnqueuedTask } from "./EnqueuedTask.ts";
...
export class Client {
   ...
   */
  async getIndex<T extends Record<string, any>>(
    indexUid: string,
  ): Promise<Index<T>> {
...
export class MeiliSearch extends Client {
  tokens: Token;
